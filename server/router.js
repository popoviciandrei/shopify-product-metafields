...
async function processPayment(ctx, next) {
    console.log("Process payment", ctx);
    if (ctx.query.charge_id) {
        ...
        fetch(`https://${ctx.session.shop}/${chargeUrl}/${ctx.query.charge_id}.json`, optionsWithGet)
            .then((response) => response.json())
            .then((myJson) => {
                if (myJson.recurring_application_charge.statue === 'accepted') {
